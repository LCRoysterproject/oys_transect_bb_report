...
total_days_26 <- length(unique(org_data$date[org_data$date > "2022-08-01"]))
```

# Overview
This report provides summary statistics and figures for ongoing transect sampling. The first section of the report focuses on the current sampling (Winter 2022-2023) and how the collected data compare to last year's sampling (Winter 2021-2022). So far `r total_days_26` days have been sampled this season. The second half of the report gives summaries of all of the data that have been collected since the beginning of the project (`r head(org_data$date, 1)`). In total, `r total_days` days have been sampled over this entire project. 
...
## Summary Tables for Periods 20, 22, 24, and 26
These summary tables provide summary statistics on live counts and oyster densities for just periods **20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023).**   
...
Data are aggregated by station and period and then summarized in the tables below. Live counts are the number of live oysters summarized by locality, strata, and period, and density is the number of live oysters per square meter summarized by locality, strata, and period. 

### Summary of Live Counts for Periods 20, 22, 24, and 26
```{r 1824_tables, echo = FALSE, comment = NA}
p2026 <- subset(cal_den, cal_den$period == 20 | cal_den$period == 22 | cal_den$period == 24 | cal_den$period == 26)
options(width = 100) #used to make the page size "larger" in rmd to fit all the data from the tables

summaryCounts(p2026)
summaryDensity(p2026)
```
\newpage

### Summary of Dead Counts for Periods 20, 22, 24, and 26
```{r dead1824_tables, echo = FALSE, comment = NA}
p2026 <- subset(cal_den, cal_den$period == 20 | cal_den$period == 22 | cal_den$period == 24 | cal_den$period == 26)

options(width = 100) #used to make the page size "larger" in rmd to fit all the data from the tables

summarydeadCounts(p2026)
summarydeadDensity(p2026)
```
\newpage
## Summary Plots for Periods 20, 22, 24, and 26
...
current_plot$period <- factor(current_plot$period, levels = c("20", "22", "24", "26"))
current_plot %>% 
  filter(period== 20 | period== 22 | period==24 | period==26) %>% 
  ggplot( aes(y= density, fill=as.factor(period))) +
  geom_boxplot()+
  ylab("Oyster density per m^2")+
  labs(title= "Live Oyster Density by Locality for Periods 20, 22, 24, and 26", fill= "Period") +
...
```
Figure- Calculated live oyster density by locality for periods 20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023) with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  ...
  geom_boxplot() +
  ylab("Oyster density per m^2")+
  labs(title= "Dead Oyster Density by Locality for Periods 20, 22, 24, and 26", fill= "Period") +
  scale_fill_manual(values=c("#CC79A7", "#E69F00", "#56B4E9", "#F0E442"))+
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid")) +
  facet_wrap(~locality)
```
Figure- Calculated dead oyster density by locality for periods 20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023) with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  ...
  geom_boxplot() +
  ylab("Oyster density per m^2")+
  labs(title= "Live Oyster Density by Strata for Periods 20, 22, 24, and 26", fill= "Period") +
  scale_fill_manual(values=c("#CC79A7", "#E69F00", "#56B4E9", "#F0E442"))+
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid")) +
    facet_wrap(~strata, ncol= 2)

```
Figure- Calculated live oyster density by strata for  periods 20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023) with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  ...
  geom_boxplot()+ 
  ylab("Oyster density per m^2")+
  labs(title= "Dead Oyster Density by Strata for Periods 20, 22, 24, and 26", fill= "Period") +
  scale_fill_manual(values=c("#CC79A7", "#E69F00", "#56B4E9", "#F0E442"))+
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid")) +
    facet_wrap(~strata, ncol= 2)
```
Figure- Calculated dead oyster density by strata for periods 20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023) with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
cal_den %>% 
  filter(period== 24 | period==26) %>% 
ggplot(aes(x=density, fill = as.factor(period))) +
          geom_density(size = 1.5, alpha = 0.5)+
          xlab("Oyster density per m^2")+
          ylab("Probability Density Function")+
          labs(title = "Live Oyster Density by Period (24 and 26)", fill = "Period")+
          scale_fill_manual(values=c("#56B4E9", "#F0E442"))+
          theme(panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))

```
Figure- Calculated live oyster density by periods 24 (Winter 2021-2022) and 26 (Winter 2022-2023) using a probability density function with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
ggplot(aes(x=density_dead, fill = as.factor(period))) +
          ...
          xlab("Oyster density per m^2")+
          ylab("Probability Density Function")+
          labs(title = "Dead Oyster Density by Period (24 and 26)", fill = "Period")+
          scale_fill_manual(values=c("#56B4E9", "#F0E442"))+
          theme(panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))
```
Figure- Calculated dead oyster density by periods 24 (Winter 2021-2022) and 26 (Winter 2022-2023) using a probability density function with the last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
data1$period <- factor(data1$period, levels = c("20", "22", "24", "26"))
data1$strata <- factor(data1$strata, levels = c("Y_Y", "Y_N", "N_Y", "N_N", "N_PILOT"))
  ...
  ggplot(aes( x= count_live, y= count_dead, color= as.factor(period))) +
    ...
    labs(title= "Live and Dead Oyster Count Comparison for Periods 20, 22, 24, and 26", color= "Period") +
    ...
    facet_wrap(~strata, ncol= 2)
```
Figure- Live and dead oyster count comparison by periods 20 (Winter 2019-2020), 22 (Winter 2020-2021), 24 (Winter 2021-2022), and 26 (Winter 2022-2023) last sample date of period 26 as `r tail(org_data$date, 1)`.
\newpage
...
#create a boxplot showing differences between pass 1 and pass 2 for periods 18,20,22,24,26
boxplot(dp2$diff ~ dp2$period, xlab = "Period", ylab = "Difference in Live Counts b/w Passes")

```
  
Figure- Boxplot of the difference in live counts between pass 1 and pass 2 (pass 1 live counts - pass 2 live counts) for period 18, 20, 22, 24, and 26
...
Figure- Boxplot of the difference in dead counts between pass 1 and pass 2 (pass 1 dead counts - pass 2 dead counts) for period 18, 20, 22, 24, and 26
...
ggplot( data= data1, aes( x= count_live, y= count_dead)) +
  ...
  theme(legend.position="right",panel.grid = element_blank(), 
    ...
    facet_wrap(~strata, ncol= 2 )
```
Figure- Live and dead oyster comparison for all periods, last sample date of period 26 is `r tail(org_data$date, 1)`.
\newpage
