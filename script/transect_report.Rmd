...
## Summary Tables for Periods 20 and 22
These summary tables provide summary statistics on live counts and oyster densities for just periods **20 (Winter 2019-2020) and 22 (Winter 2020-2021).**   
...
### Summary of Live Counts for Periods 20 and 22
```{r 2022_tables, echo = FALSE, comment = NA}
p2022 <- subset(cal_den, cal_den$period == 20 | cal_den$period == 22)
...
### Summary of Dead Counts for Periods 20 and 22
...
```
\newpage
## Summary Plots for Periods 20 and 22
  
```{r 2022_local, echo=FALSE,comment=NA}

current_plot<- cal_den

current_plot$period <- factor(current_plot$period, levels = c("22", "20"))
...
current_plot %>% 
  ...
  xlab("Locality")+
  labs(title= "Live Oyster Density by Locality for Periods 20 and 22", fill= "Period") +
  scale_fill_discrete(breaks=c("20","22"))+
...
```
Figure- Calculated live oyster density by locality for periods 20 (Winter 2019-2020) and 22 (Winter 2020-2021) with the last sample date of period 22 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  filter(period== 20 | period==22) %>% 
  ggplot( aes( x= locality, y= density_dead, fill=as.factor(period))) +
  geom_boxplot()+ coord_flip()+
  ylab("Oyster density per m^2")+
  xlab("Locality")+
  labs(title= "Dead Oyster Density by Locality for Periods 20 and 22", fill= "Period") +
   scale_fill_discrete(breaks=c("20","22"))+
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))
```
Figure- Calculated dead oyster density by locality for periods 20 (Winter 2019-2020) and 22 (Winter 2020-2021) with the last sample date of period 22 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  ...
  ylab("Oyster density per m^2")+
  xlab("Strata")+
  labs(title= "Live Oyster Density by Strata for Periods 20 and 22", fill= "Period") +
   scale_fill_discrete(breaks=c("20","22"))+
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))

```
Figure- Calculated live oyster density by strata for periods 20 (Winter 2019-2020) and 22 (Winter 2020-2021) with the last sample date of period 22 as `r tail(org_data$date, 1)`.
\newpage
...
current_plot %>% 
  filter(period== 20 | period==22) %>% 
  ggplot( aes( x= strata, y= density_dead, fill=as.factor(period))) +
  geom_boxplot()+ coord_flip()+
  ylab("Oyster density per m^2")+
  xlab("Strata")+
  labs(title= "Dead Oyster Density by Strata for Periods 20 and 22", fill= "Period") +
  scale_fill_discrete(breaks=c("20","22")) +
  theme(legend.position="right",panel.grid = element_blank(), panel.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))
```
Figure- Calculated dead oyster density by strata for periods 20 (Winter 2019-2020) and 22 (Winter 2020-2021) with the last sample date of period 22 as `r tail(org_data$date, 1)`.
\newpage
...
data1$period <- factor(data1$period, levels = c("22", "20"))
  ...
  ggplot(aes( x= count_live, y= count_dead, color= as.factor(period))) +
    ...
    labs(title= "Live and Dead Oyster Count Comparison of Periods 20 and 22", color= "Period") +
    scale_color_discrete(breaks=c("20","22")) +
    theme(legend.position="right",panel.grid = element_blank(), 
          panel.background = element_blank(), 
          panel.border = element_rect(colour = "black", fill=NA,size=1, linetype="solid"))+
    facet_wrap(~strata, ncol= 2)
```
Figure- Live and dead oyster count comparison by periods 20 (Winter 2019-2020) and 22 (Winter 2020-2021), last sample date of period 22 as `r tail(org_data$date, 1)`.
\newpage
...
boxplot(dp2$diff ~ dp2$period, xlab = "Period", ylab = "Difference in Counts b/w Passes")

```
  
Figure- Boxplot of the difference in counts between pass 1 and pass 2 (pass 1 live counts - pass 2 live counts) for period 18, 20, and 22
...
Table- Coefficient variation between pass 1 and pass 2, aggregated by locality and period 
