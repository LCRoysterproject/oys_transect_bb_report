############
## GLM Models using MASS package - glm.nb function
## Jennifer Moore
...
#import data
d <- read.csv("data/LC_transect_data_2010_2019.csv", header = T)
...
h2 = h + geom_line(aes(x = seq(0,5350,25), y = out), size = 1.5, col = 'red')
save_plot("data_output/big_bend_trends/graphs/Fig2HistDensity_updated.tiff", h2, dpi = 300)


#######################
#basic GLMM w/o covariates

glmm1 <- glm.nb(count_live ~ period + offset(log(tran_length)), data = d2) 
glmm2 <- glm.nb(count_live ~ period + locality + offset(log(tran_length)), data = d2) 
glmm3 <- glm.nb(count_live ~ period * locality + offset(log(tran_length)), data = d2)
glmm4 <- glm.nb(count_live ~ period + site + offset(log(tran_length)), data = d2) 
glmm5 <- glm.nb(count_live ~ period * site + offset(log(tran_length)), data = d2) 
glmm6 <- glm.nb(count_live ~ period + locality + site + offset(log(tran_length)), data = d2)
glmm7 <- glm.nb(count_live ~ period + locality * site + offset(log(tran_length)), data = d2) 
glmm8 <- glm.nb(count_live ~ period * locality + site + offset(log(tran_length)), data = d2) 
glmm9 <- glm.nb(count_live ~ period * locality * site + offset(log(tran_length)), data = d2) 
glmm10 <- glm.nb(count_live ~ period * site + locality + offset(log(tran_length)), data = d2) 


cand.set = list(glmm1, glmm2, glmm3, glmm4, glmm5, glmm6, glmm7, glmm8, glmm10)
modnames = c("period", "period + locality", "period * locality", "period + site", "period * site", "period + locality + site", "period + locality * site", "period * locality + site", "period * site + locality")
...
# add the legend to the row we made earlier. Give it one-third of 
# the width of one plot (via rel_widths).
pm2 = plot_grid(pm, legend, rel_widths = c(2.5,.4))


save_plot("data_output/big_bend_trends/graphs/Fig3GLMNB_glmm8_final.tiff", pm2, dpi = 300, base_width = 10, base_height = 7)
...
glmm11 <- glm.nb(count_live ~ period * locality + site + ADsc + offset(log(tran_length)), data = d14) 
#river discharge 1 year lag (yearly mean)
glmm12 <- glm.nb(count_live ~ period * locality + site + ADLag1sc + offset(log(tran_length)), data = d14) 
#river discharge 2 year lag (yearly mean)
glmm13 <- glm.nb(count_live ~ period * locality + site + ADLag2sc + offset(log(tran_length)), data = d14) 
#river discharge year of sampling (yearly total)
glmm14 <- glm.nb(count_live ~ period * locality + site + TDsc + offset(log(tran_length)), data = d14) 
#river discharge 1 year lag (yearly total)
glmm15 <- glm.nb(count_live ~ period * locality + site + TDLag1sc + offset(log(tran_length)), data = d14) 
#river discharge 2 year lag (yearly total)
glmm16 <- glm.nb(count_live ~ period * locality + site + TDLag2sc + offset(log(tran_length)), data = d14) 
#oyster meat weight (landings) year of sampling
glmm17 <- glm.nb(count_live ~ period * locality + site + Landingssc + offset(log(tran_length)), data = d14) 
#oyster meat weight 1 year lag
glmm18 <- glm.nb(count_live ~ period * locality + site + LandingsLag1sc + offset(log(tran_length)), data = d14) 
#oyster meat weight 2 year lag
glmm19 <- glm.nb(count_live ~ period * locality + site + LandingsLag2sc + offset(log(tran_length)), data = d14) 
#oyster fishing trips year of sampling
glmm20 <- glm.nb(count_live ~ period * locality + site + Tripssc + offset(log(tran_length)), data = d14) 
#oyster fishing trips 1 year lag
glmm21 <- glm.nb(count_live ~ period * locality + site + TripsLag1sc + offset(log(tran_length)), data = d14) 
#oyster fishing trips 2 year lag
glmm22 <- glm.nb(count_live ~ period * locality + site + TripsLag2sc + offset(log(tran_length)), data = d14) 
#oyster fishing (yes/no) 
glmm23 <- glm.nb(count_live ~ period * locality + site + fish + offset(log(tran_length)), data = d14)

cand.set = list(glmm8, glmm11, glmm12, glmm13, glmm14, glmm15, glmm16, glmm17, glmm18, glmm19, glmm20, glmm21, glmm22, glmm23)
modnames = c("Base", "AD", "ADLag1", "ADLag2", "TD", "TDLag1", "TDLag2", "Landings", "LandingsLag1", "LandingsLag2", "Trips", "TripsLag1", "TripsLag2", "Harvest")
aictab(cand.set, modnames, second.ord = FALSE) #model selection table with AIC
...
p3 = ggplot(test, aes(x = x_unscaled, y = predicted)) +
  ...
  xlim(3500,12500) +
  labs(y = 'Oyster Counts (per 1 m)', x = 'Mean Daily Discharge (cf/s)') +
  theme_classic() +
  theme(text = element_text(size=15))


p3
save_plot("data_output/big_bend_trends/graphs/Fig7MeanDischarge_final.tiff", p3, dpi = 300)
...
mod <- list()
test <- ggpredict(glmm8, terms = c('period'), type = c('fe'), data = d14)
plot(test$x, test$predicted, type = 'l', xlab = "Period", ylab = "Live Oyster Counts", col = 'blue', ylim = c(0,500))

for(i in 1:nsim){
  m <- glm.nb(count2 ~ period * locality + site + offset(log(tran_length)), data = sim[[i]])
  period[i] <- coef(m)[2]
  pval[i] <- summary(m)$coefficients[2,4]
...
p #display plot
save_plot("data_output/big_bend_trends/graphs/Fig1SimOutputs_final.tiff", p, dpi = 300)


#create density plot for pvalue
tiff("data_output/big_bend_trends/graphs/Fig2SPvalDensity_final.tiff", res = 300, units = 'px', height = 3200, width = 3200)
par(mar = c(5.1, 4.5, 5.1, 2.1))
plot(density(pval), main = "", xlab = "p-value", cex.axis = 1.5, lwd = 3, cex.lab = 2)
dev.off()
