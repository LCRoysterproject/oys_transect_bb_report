...
#I'm going to take day and month and year out here (and other aggregates) because in a couple
#of instances transected were counted same bar two different days
#this is just winter 2018/2019

#live=aggregate(count_live~day+month+year+season+treatment+locality+site+bar+station,data=dta0.2,sum)
live=aggregate(count_live~season+treatment+locality+site+bar+station,data=dta0.2,sum)
...
dta2=aggregate(tran_length~season+treatment+locality+site+bar+station+transect,data=dta0,max)
...
tranlength=aggregate(tran_length~season+treatment+locality+site+bar+station,data=dta2,sum)
...
dta3=merge(live,tranlength,by=c("season","treatment","locality","site","bar","station"))
...
sort.dta3<-dta3[order(dta3$station),]
